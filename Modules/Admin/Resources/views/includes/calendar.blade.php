                                    ...
                                    @if ($items = $employee->availableTimes($cDate->toDateString()))
                                        @foreach ($items as $item)
                                            <section class="item__block item__block--calendar">
                                                <div class="item__name">{{ firstChar($employee->full_name, true) }} {{ $employee->full_name }}</div>
                                ...
                                @else
                                    @foreach (App\Models\User::all() as $employee)
                                        @if ($items = $employee->availableTimes($cDate->toDateString()))
                                            @foreach ($items as $item)
                                                <section class="item__block item__block--calendar">
                                                    <div class="item__name">{{ firstChar($employee->full_name, true) }} </div>
                                                    <div class="item__time">{{ toTime($item['start_time'], false) }} - {{ toTime($item['end_time'], false) }}</div>
                                                </section>
                                            @endforeach
                                        @endif
                            ...
                            <div class="item">
                                @foreach ($items as $item)
                                    @if ($item->date == $cDate->toDateString())
                                        <section class="item__block  item__block--calendar item__block--padding" data-toggle="tooltip" data-placement="top" title="{{ $item->employee->full_name }} {{ $item->employee->full_name }} - {{ $item->employee->title }}">
