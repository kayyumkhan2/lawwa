...
<div class="card">
   ...
   <div class="card-body">
      <form action="{{ route('admin.page.academy.certificates') }}" enctype="multipart/form-data" method="POST" id="formid" name="formid">
         {{ csrf_field() }}
         ...
         <div class="form-group row">
            ...
            <label for="nf-name">Feature  </label>
              <input type="text" class="form-control"  placeholder="Enter feature name" required="" required="" name="features[]">
            </div>
                  ...
                  <table id="dataTable" class="table table-striped table-bordered table-hover">
                     ...
                     <tbody>
                        ...
                        <tr class="notification{{$value->id}}">
                           ...
                           <td>@foreach($value->CertificateFeature as $features){{$features->feature}} <br> @endforeach</td>
                           <td><img src="{{asset('images/frontpages/certificates/'.$value->image)}}" class="img-thumbnail" width="80"></td>
                           <td>
                              <span class="Statuschange badge @if($value->status=='0') badge-danger @else badge badge-success @endif" data-id="{{$value->id}}"  data-model="Certificate" id="Statuschange{{$value->id}}">
                              @if($value->status=="0") Deactive @else Active @endif 
                              </span>
                           </td>
                           <td class="action ">
                              <a class="icon-btn delete universaldelete" href="javascript:void(0);" data-status="0" data-id="{{ $value->id}}"  data-model="Certificate" id="notification{{$value->id}}" > <i class="fal fa-trash-alt" id="delete-btn"></i></a> 
                           </td>
                        </tr>
                        @endforeach
                     </tbody>
...
<script type="text/javascript">
   ...
   $(document).on('click','#add',function(){
         i++;
        // alert(i);
         var data='<div class="col-md-11 newrow'+i+'"><div class="form-group mt-3"><input type="tel" class="form-control mobile"  data-name="'+i+'" name="features[]" placeholder="Enter feature name"></div></div><div class="col-md-1 newrow'+i+'" data-id="'+i+'" id="minues" style="font-size: 48px;"><i class="far fa-minus-square text-danger"></i></div>';
           $(this).after(data);
       });
